...
static char key[TF_KEY_SIZE], xtskey[TF_KEY_SIZE], ctr[TF_BLOCK_SIZE];
...
{
	printf("usage: tfcrypt <-e/-d/-we/-wd> key srcfile dstfile.\n");
	printf("-e: encrypt, -d: decrypt.\n");
	printf("-we: encrypt with skein, -wd: decrypt with skein.\n");
	printf("Encrypts srcfile into dstfile with key using XTS mode.");
